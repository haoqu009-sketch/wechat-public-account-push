...
配置了 `TIAN_API_KEY` 后，脚本会自动优先调用天行天气预报接口，使用 `USER_INFO` 中的 `city` 查询真实天气、最高/最低温、风向和风力。为保证微信测试模板稳定、完整地展示每日一句与诗词，天气区域只保留紧凑摘要，不追加天气提醒。`weatherCityCode` 不再是必填项，但建议保留，便于天行接口无权限或临时不可用时回退原天气源。
