...
为青龙面板设计的单文件微信公众号推送脚本，经过重构和优化，支持微信测试号和 PushDeer 推送。
...
1.  将 `wechat-push.js` 文件放置在青龙面板的脚本目录下（例如 `ql/scripts/`）。
...
脚本允许微信模板自动换行展示中长内容：中文最多 60 字符、英文最多 180 字符；仅在接口异常、内容为空、含不适宜词汇或异常超长时自动使用本地 365 组中英句库。出处会记录在运行日志中，英文原句和中文释义会完整推送。

### 天行数据天气预报

配置了 `TIAN_API_KEY` 后，脚本会自动优先调用天行天气预报接口，使用 `USER_INFO` 中的 `city` 查询实时天气、最高/最低温、风向、风力、紫外线与空气质量。再配置 `DEEPSEEK_API_KEY`，即可把这些真实数据改写为两行自然的生活提醒。天气摘要使用 `{{b.DATA}}`，两行提醒分别使用 `{{b2.DATA}}`、`{{b3.DATA}}`，每个变量都限制在微信安全显示长度内。`weatherCityCode` 不再是必填项，但建议保留，便于天行接口无权限或临时不可用时回退原天气源。

推荐使用以下完整模板。短标签由代码写入变量值，以降低微信模板源码长度，防止尾部的每日一句和诗句变量在保存时被裁掉：
