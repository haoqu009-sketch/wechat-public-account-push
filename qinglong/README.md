...
配置了 `TIAN_API_KEY` 后，脚本会自动优先调用天行天气预报接口，使用 `USER_INFO` 中的 `city` 查询实时天气、最高/最低温、风向、风力与生活提示并填入基础模板字段。天气栏会显示一行摘要和最多两行提醒，无需修改现有微信模板。`weatherCityCode` 不再是必填项，但建议保留，便于天行接口无权限或临时不可用时回退原天气源。
